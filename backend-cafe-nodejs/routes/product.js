...
router.post('/add',auth.authenticateToken,checkRole.checkRole,(req, res)=>{
    ...
    connection.query(query,[product.name,product.categoryId ,product.description,product.price],(err, results)=>{
        console.log([product.name,product.categoryId ,product.description,product.price]);
        if(!err){
...
router.delete('/detete/:id',auth.authenticateToken,checkRole.checkRole,(req, res,next)=>{
    const id = req.params.id;
...
router.patch('/updateStatus',auth.authenticateToken, checkRole.checkRole,(req,res, next)=>{
    ...
    connection.query(query,[user.status, user.id],(err, results)=>{
        if(!err){
            if(results.affectedRows == 0){
                return res.status(404).json({message: "Prouct id does not found"});
            }
